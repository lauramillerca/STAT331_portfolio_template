...
[**My Grade:**]{.underline} I believe my grade equivalent to course work evidenced below to be a \_B\_.
...
-   Date (example must use functions from **lubridate**): Lab 5 2nd to last step of suspect investigation – revised to change as.Date to ymd
...
-   Numeric Example 1: Lab 5 2nd to last step of suspect investigation - revised to change as.Date to ymd
...
-   `right_join()` Example 1: Lab 5 2nd to last step of suspect investigation – revised to change inner_join() to right_join()
...
-   `inner_join()` Example 1: Lab 5 2nd to last step of suspect investigation
...
-   I can modify my plot titles to clearly communicate the data context: Challenge 2 Package Specification - revised to modify axis labels and title
...
ggplot(data = surveys, mapping = aes(x = species, y = weight, color = sex)) +
  ...
  geom_boxplot(outlier.shape = NA) +
  labs(x = "Species of Animal", y = "Weight of Animal (g)", title = "Distribution of Animal Weight by Species") +
...
-   I can use non-standard colors (Example 1): Challenge 2 Package Specification
...
-   Example using `across()` : Lab 8 Question 1
...
-   I can modify the text in my table to be more readable (e.g., bold face for column headers): Lab 4 Question 6 - revised to make bold column headers and better names
...
-   I can arrange my table to have an intuitive ordering: Lab 4 Question 5 - revised to make better names
...
med_income_by_region <- ca_childcare |>
  ...
  select(census_region, median_income_2008, median_income_2018) |>
...
-   I can modify the layout of my table to be more readable (e.g., `pivot_longer()` or `pivot_wider()`) : Challenge 4 Summary Statistics
...
-   Example 2: Function that operates on data frames: Lab 8 Question 3
...
-   Example 3: Function that operates on vectors *or* data frames: Lab 8 Question 2
...
-   I can use functions which are not superseded or deprecated: Lab 5 Last step of finding the suspect
